...
\ifXeTeX
\special{pdf:trailerid [ <cc15bd2d28082a1f75e183d2f4218fb4> <cc15bd2d28082a1f75e183d2f4218fb4> ]}
\fi
\ifPDFTeX
\pdftrailerid{}
\pdftrailer{/ID [ <cc15bd2d28082a1f75e183d2f4218fb4> <cc15bd2d28082a1f75e183d2f4218fb4> ]}
\fi
\ifLuaTeX
\pdfvariable trailerid {[ <cc15bd2d28082a1f75e183d2f4218fb4> <cc15bd2d28082a1f75e183d2f4218fb4> ]}
\fi
...
\subsection{A chunk that runs and produces a
number}\label{a-chunk-that-runs-and-produces-a-number}
...
\subsection{A chunk that runs and produces a
figure}\label{a-chunk-that-runs-and-produces-a-figure}

Also executed at build time. The figure is generated, not a file
committed to the repository --- which is the whole point of the
exercise, and the thing that distinguishes these notebooks from the
projects.
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Three curves decaying from one towards zero, the steepest labelled k equals one and the shallowest k equals one quarter.}]{python-test-drive_files/figure-latex/fig-decay-output-1.pdf}}

}

\caption{\label{fig-decay}Exponential decay at three rates. Generated at
build time by matplotlib.}

\end{figure}%

\subsection{A chunk that is complete, but left for you to
run}\label{a-chunk-that-is-complete-but-left-for-you-to-run}

The code below is finished and correct. It is deliberately \emph{not}
...
Open this notebook's \texttt{.ipynb} version, put your cursor in the
cell, and run it.
...
\subsection{A chunk that is yours to
write}\label{a-chunk-that-is-yours-to-write}

Yours to fill in. Plot \(\sin(t)\) against \(t\) over the same range as
the figure above, using whatever you can remember or work out from the
second chunk.
...
If the pipeline is working, then in the \texttt{html} and \texttt{pdf}
versions of this page the first two chunks show their results and the
last two show only code. In the \texttt{.ipynb} version all four arrive
as runnable code cells, and ``Run All'' completes without error.
